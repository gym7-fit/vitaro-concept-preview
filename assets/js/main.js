/* ---------- SITE CONFIG (the single place to switch things live) ----------
   BOOKING_URL_DE/EN   Cal.com / Calendly link, one per language (EN also
                       serves ES visitors). Picked by document.documentElement.lang
                       every time applyLang() runs (see updateBookingLinks() below
                       and the hook at the bottom of i18n.js), so it stays correct
                       across language switches with no page reload. Both empty =
                       every [data-booking-link] keeps its fallback href (the
                       contact form) and the kontakt page hides its booking button.
...
var BOOKING_URL_DE = 'https://calendly.com/vitaro/20min-de';
var BOOKING_URL_EN = 'https://calendly.com/vitaro/20min-en';
...
function updateBookingLinks(){
  ...
  var url = (lang === 'de' && BOOKING_URL_DE) ? BOOKING_URL_DE : (BOOKING_URL_EN || BOOKING_URL_DE || '');
  ...
  if(bookingBtn){ bookingBtn.hidden = !url; }
}
window.updateBookingLinks = updateBookingLinks;
updateBookingLinks();
